...
class SNResNetProjectionDiscriminator(nn.Module):
    ...
    def __init__(self, num_features=64, num_classes=0, activation=F.relu):
        ...
        self.block5 = Block(num_features * 8, num_features * 8,
                            activation=activation, downsample=True)
        self.block6 = Block(num_features * 8, num_features * 16,
                            activation=activation, downsample=True)
        self.block7 = Block(num_features * 16, num_features * 16,
                            activation=activation, downsample=True)
        self.l8 = utils.spectral_norm(nn.Linear(num_features * 16, 1))
        if num_classes > 0:
    ...
    def _initialize(self):
        init.xavier_uniform_(self.l8.weight.data)
