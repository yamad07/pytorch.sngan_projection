...
class ResNetGenerator(nn.Module):
    ...
    def __init__(self, num_features=64, dim_z=128, bottom_width=4,
        ...
        self.block3 = Block(num_features * 16, num_features * 8,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block4 = Block(num_features * 8, num_features * 8,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block5 = Block(num_features * 8, num_features * 4,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block6 = Block(num_features * 4, num_features * 2,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block7 = Block(num_features * 2, num_features,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.b8 = nn.BatchNorm2d(num_features)
        self.conv8 = nn.Conv2d(num_features, 3, 1, 1)
